...
@Author : Hishallyi
@Date   : 2024/6/27
@Code   : 相机拍摄流程——双线程控制两个GUI(CamDisplayGUI和CamSettingThread)，双线程运行在主线程中 报错！！！
@problem: 俩线程无法交互，点击tkinter后两界面卡住
"""
...
class CamSettingThread(threading.Thread):
    def __init__(self, camera_sn_list):
        threading.Thread.__init__(self)
        self.camera_sn_list = camera_sn_list
    ...
    def run(self):
        ...
        camera_option = tk.OptionMenu(root, camera_list, *self.camera_sn_list)
...
tkinter_thread = CamSettingThread(camera_sn_list)
